...
public final class MainActivity extends Activity {
    ...
    private void render() {
        ...
        header.addView(add,PocketUi.lp(this,48,45,0,0,0,0));
    ...
    void scan() {
        ...
        new Thread(()->{
            LibraryStore.ScanReport report=library.scan();scanning.set(false);
            runOnUiThread(()->{
                render();
                String message=report.added+" novo(s), "+report.found+" encontrado(s).";
                if(report.errors>0)message+=" "+report.errors+" falha(s); biblioteca preservada.";
                new AlertDialog.Builder(this).setTitle("Importação concluída")
                    .setMessage(message+(report.warnings.isEmpty()?"":"\n\n"+String.join("\n",report.warnings)))
                    .setPositiveButton("OK",null).show();
            });
        },"Pocket-Rom-Scan").start();
    }
    void showActions(LibraryStore.Game game) {
        String[] options={"Jogar","Informações","Renomear","Favoritar / desfavoritar",game.hidden?"Restaurar à biblioteca":"Remover da lista"};
    ...
    void details(LibraryStore.Game game) {
        String info="Sistema: "+game.system+"\nArquivo: "+game.fileName+"\n\n"
            ...
            +"\n\nAs ROMs e seus saves não são excluídos ao remover da lista.";
    ...
    void removeRoot(LibraryStore.Root root) {
        new AlertDialog.Builder(this).setTitle("Desvincular pasta?")
            ...
            .setNegativeButton("Cancelar",null).setPositiveButton("Desvincular",(d,w)->{
                library.removeFolder(root);render();
            }).show();
    }
